...
De eerste methode die we nodig hebben, is de methode waarmee we een stuk op een specifieke positie op het bord kunnen plaatsen. Hiervoor definiëren we de methode `put_piece_on_position`, die een stuk op de opgegeven positie zet. De methode neemt twee argumenten:
...
```
>>> board.put_piece_on_position(Piece('Qw'), 'd1')
```
...
Dan hebben we natuurlijk ook een methode nodig om te zien welk stuk zich op een bepaalde positie op het bord bevindt. Hiervoor definiëren we de methode `get_piece_by_position`, die hetzelfde soort positie-argument gebruikt als `put_piece_on_position`.

```
>>> piece = board.get_piece_by_position('d1')
>>> piece.get_code()
"Qw"
```
...
```
>>> board = Board()
>>> board.get_piece_by_position('d1')
"Qw"
>>> board.get_piece_by_position('d8')
"Qb"
>>> board.get_piece_by_position('c2')
"pw"
>>> board.get_piece_by_position('c8')
"Nb"
>>> board.get_piece_by_position('d4')
None
(etc)
```
...
Beide positie-argumenten volgen hetzelfde stringformaat als in `get_piece_by_position` en `put_piece_on_position`.
...
```
>>> board.get_piece_by_position('f2')
None
```
en de pion nu in de plaats op f4 staat:
```
>>> piece = board.get_piece_by_position('f4')
>>> piece.get_code()
"pw"
```
...
```
>>> board.get_piece_by_position('e5').get_code()
"pb"
```
2. en dat f4 door een witte pion wordt bezet
```
>>> board.get_piece_by_position('f4')
```
