...
Het moet ook mogelijk zijn om de statistieken op te vragen. Hiervoor schrijven we een methode `get_statistics()`, die alle statistieken van een speler zal teruggeven.

```python
>>> stats = lbj.get_statistics()
>>> stats["FGA"]
0
>>> stats["FGM"]
0
...
Om het resultaat van deze methode te zien, moeten we ook een methode `get_player_by_number` voorzien. Deze methode ontvangt een spelersnummer als argument en op basis daarvan geeft hij het `Player` object terug dat binnen dit team daarmee gelinked is.

```python
>>> lbj = lakers.get_player_by_number(23)
>>> lbj.name
"Lebron James"
>>> lbj.number
23
...
Om het resultaat van de vorige methode te zien, moeten we ook een methode `get_team_by_abbrev` voorzien. Deze methode ontvangt een teamafkorting als argument en op basis daarvan geeft hij het `Team` object terug dat binnen deze match daarmee gelinked is.

```python
>>> team = match.get_team_by_abbrev("LAL")
>>> team.name
"LA Lakers"
```

### Toon alle teams uit een match
We zullen ook een methode `get_teams` voorzien in `Match`, dewelke een lijst teruggeeft van alle teams die behoren tot deze match.
