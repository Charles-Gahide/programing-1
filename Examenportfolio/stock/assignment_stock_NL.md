...
Om het resultaat van de bovenstaande methode te kunnen zien, moeten we ook een extra methode `get_part_by_name` voorzien. Deze methode ontvangt een naam van een onderdeel als argument en op basis daarvan geeft die het corresponderende `Part` object uit onze stock terug.

```python
>>> part = stock.get_part_by_name("WD Red Plus 12TB")
>>> part.name
"WD Red Plus 12TB"
```
...
Verder zullen we ook een methode `get_parts` voorzien op ons `Stock` object. Deze methode zal alle onderdelen uit onze stock teruggeven in een lijst.
...
Om aan onze private field stock te kunnen, voorzien we ook een methode `get_stock()`:
